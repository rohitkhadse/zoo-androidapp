...
public class Spider {
    ...
    SpiderState state;			// current state of bug
    ...
    public void birth (Canvas canvas) {
        ...
        if (state == SpiderState.Dead) {
            ...
            state = SpiderState.ComingBackToLife;
        ...
        // Check if bug is alive yet
        else if (state == SpiderState.ComingBackToLife) {
            ...
            if (curTime - startBirthTimer >= timeToBirth) {
                ...
                else if (x < Assets.spider2.getWidth()/2)
                    x = Assets.spider2.getWidth()/2;
                else if (x > canvas.getWidth() - Assets.spider2.getWidth()/2)
                    x = canvas.getWidth() - Assets.spider2.getWidth()/2;
                y = 0;
    ...
    public void move (Canvas canvas) {
        ...
        if (state == SpiderState.Alive) {
            ...
            //while ( state == SpiderState.Alive) {
                y += (speed * elapsedTime);
                // Draw bug on screen
                if (Assets.spider_boolean == false) {
                    canvas.drawBitmap(Assets.spider1, x, y, null);
                    Assets.spider_boolean = true;
                } else if (Assets.spider_boolean) {
                    canvas.drawBitmap(Assets.spider2, x, y, null);
                    Assets.spider_boolean = false;
                }
            //}
    ...
    public boolean touched (Canvas canvas, int touchx, int touchy) {
        ...
        if (state == SpiderState.Alive) {
            ...
            if (dis <= Assets.spider1.getWidth()*0.75f || dis <= Assets.spider2.getWidth()*0.75f) {
                state = SpiderState.DrawDead;	// need to draw dead body on screen for a while
                touched = true;
                if (touched){
                Assets.newScore++;
                    Assets.assetcount++;
    ...
    static public void randomSound(){
        Random randomGenerator = new Random();
        for (int idx = 1; idx <= 1; ++idx){
            int randomInt = randomGenerator.nextInt(3);
            Assets.randomInt = randomInt;
        }
    }
